...
SeamCarver.prototype.initHeatMap = function () {
    ...
    var me = this; var b = function (x, y) {
    ...
    var heatMap = [];
    var max = 0; for (var x = 0; x < this.image.width; x++) {
        heatMap[x] = [];
        for (var y = 0; y < this.image.height; y++) {
...
SeamCarver.prototype.initSeams = function () {
    ...
    for (var i = 0; i < yseam.length; i++) {
        for (var j = i + 1; j < yseam.length; j++) {
            if (this.heatMap[yseam[i][ylen]][ylen] > this.heatMap[yseam[j][ylen]][ylen]) {
                var tmp = yseam[j]; yseam[j] = yseam[i]
                yseam[i] = tmp;
            }
        }
    }
    for (var x = 0; x < yseam.length; x++) {
        for (var y = ylen - 1; y >= 0; y--) {
            var x1 = yseam[x][y + 1]; var x0 = x1 - 1; while (x0 >= 0) {
                if (!isNaN(this.heatMap[x0][y]))
                    break; x0--;
            }
            var x2 = x1 + 1; while (x2 < this.image.width) {
                if (!isNaN(this.heatMap[x2][y]))
                    break; x2++;
            }
            var hx0 = this.heatMap[x0] ? this.heatMap[x0][y] : Number.MAX_VALUE; var hx1 = this.heatMap[x1][y] || Number.MAX_VALUE; var hx2 = this.heatMap[x2] ? this.heatMap[x2][y] : Number.MAX_VALUE; yseam[x][y] = hx0 < hx1 ? (hx0 < hx2 ? x0 : x2) : (hx1 < hx2 ? x1 : x2); this.heatMap[yseam[x][y]][y] = NaN;
        }
    }
    log("Seams calculated"); this.seams = yseam; return this;
}
SeamCarver.prototype.getHeatMap = function () {
    this.heatMap || this.initHeatMap(); this.copyImage(); for (var x = 0; x < this.image.width; x++) { for (var y = 0; y < this.image.height; y++) { var color = parseInt(this.heatMap[x][y] / this.maxHeat * 255); this.putPixel(x, y, { "red": color, "blue": color, "green": color, "alpha": 1 }); } }
    return this.newImage;
}
SeamCarver.prototype.getSeams = function () {
    this.seams || this.initSeams(); this.copyImage(); var color = 255; var step = 1; for (var x = 0; x < this.seams.length; x++) {
        for (var y = 0; y < this.image.height; y++) { this.putPixel(this.seams[x][y], y, parseInt("0x" + color + color + color, 16)); }
    ...
    for (var y = 0; y < image.height; y++) {
        ...
        for (var x = 0; x < image.width; x++) {
            ...
            if (isSkippable == false) {
                this.putPixel(x1, y, this.getPixel(x, y)); x1++;
            }
        }
    }
